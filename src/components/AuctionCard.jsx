...
const AuctionCard = ({ auction }) => {
  ...
  return (
    <Link to={`/auction/${id}`}>  {/* Link to single auction page */}
      <div className="auction-card">
        ...
        <div className="auction-details">
          ...
          <p className="ending-time">{formattedEndingTime ? `Ending: ${formattedEndingTime}` : 'Ending Soon'}</p>
        </div>
      </div>
    </Link>
  );
...
// Function to format ending time
function formatTime(endingTime) {
  ...
  return formattedTimeString.trim(); // Remove trailing whitespace
}
